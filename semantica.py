...
class Semantica():
    ...
    def __init__(self, codigo):
        self.table = {}
        self.scope = "global"
        self.tree = parser. parse_tree(codigo)
...
#     p[0] = Tree('func_decl', [p[1], p[4], p[7]], [p[2]])
    ...
    def func_decl(self, node):
        if(node.value[0] is self.table.keys()) : #se ja tem funcao com esse nome na tabela
            print("Erro Semântico, o nome "+  node.value[0] + " já esta sendo utilizado")
            exit(1)

        self.scope = node.value[0]  #escopo nome da função
        self.parametro_decl(node.child[1])
        self.sequencia_decl(node.child[3])

        self.table[node.value[0]] = {}
        self.table[node.value[0]]["var"] = false
        self.table[node.value[0]]["tipo"] = self.tipo(node.child[0])
...
#     p[0] = Tree('sequencia_decl_loop', [p[1], p[2]])
    ...
    def sequencia_decl(self, node):
        if(len(node.child) == 2) :
            self.sequencia_decl(node.child[0])
            self. declarao(node.child[1])
        else :
            self. declarao(node.child[0])

    def declaracao(self, node) :
        if(node.child[0].type == "declaracao_se") :
            self.se_decl(node.child[0])

        elif(node.child[0].type == "declaracao_repita") :
            self.repita_decl(node.child[0])

        elif(node.child[0].type == "declaracao_atribuicao") :
            self.atribuicao_decl(node.child[0])

        elif(node.child[0].type == "declaracao_leia") :
            self.leia_decl(node.child[0])

        elif(node.child[0].type == "declaracao_escreva") :
            self.escreva_decl(node.child[0])

        elif(node.child[0].type == "declaracao_declaravar") :
            self.declara_var(node.child[0])

        elif(node.child[0].type == "declaracao_retorna") :
...
#     p[0] = Tree('retorna_numero', [p[3]])
    ...
    def retorna_declaracao(self, node) :
        if (node.child[0].type == "retorna_numero") :
            self.numero_decl(node.child[0])
        else :
            return node.value[0] #retorna o id
...
#     p[0] = Tree('leia_decl', [], [p[3]])
    ...
    def leia_decl(self, node) :
        if (self.scope + "." + node.value[0] not in self.table.keys()) :
            print("Erro Semântico. Variável " + node.value[0] + " não encontrada")
            exit(1)
...
#     p[0] = Tree('chamada_func', [p[3]], [p[1]])
    ...
    def chamada_func(self, node) :
        ...
        if(node.value[0] not in self.table.keys()) :
            print("Erro Semântico, nome de funcao não declarado : " + node.value[0] )
            exit(1)

        self.parametro_chama_func(node.child[0])
...
#     p[0] = Tree('parametro_chama_func_paramentro', [p[1]], [p[2]])

    def parametro_chama_func(self, node) :
        if ( node.child[0].type == "parametro_chama_func_num") :
            self.numero_decl(node.child[0])

        else (node.child[0].type == "parametro_chama_func")
            self.parametro_chama_func(node.child[0])
...
#     p[0] = Tree('fator_exp', [p[2]])
    ...
    def fator(self, node) :
        if(node.child[0].type == "fator_numero") :
            self.numero_decl(node.child[0])

        elif  (node.child[0].type == "fator_exp") :
            self.exp_decl(node.child[0])

        else :
            #verificar se o fator esta inicializado
...
#     p[0] = Tree('declara_var', [p[1]], [p[3]])
    ...
    def declara_var(self, node) :
        if(self.scope + "." + node.value[0] is self.table.keys()): #se ja tem variavel com esse nome na tabela
            print("Erro Semântico, nome já utilizado : " + node.value[0] )
            exit(1)

        self.table[self.scope + "." + node.value[0]] = {}
        self.table[self.scope + "." + node.value[0]]["var"] = true
        self.table[self.scope + "." + node.value[0]]["tipo"] = self.tipo(node.child[0])
...
#     p[0] = Tree('parametro_decl', [p[1]], [p[3]])
    ...
    def parametro_decl(self, node) :
        if(node.chil
            d[0].type == "parametro_decl_loop") :
            self.parametro_decl(node.child[0])

            self.table[self.scope + "." + node.value[0]] = {}
            self.table[self.scope + "." + node.value[0]]["var"] = true
            self.table[self.scope + "." + node.value[0]]["tipo"] = self.tipo(node.child[1])
